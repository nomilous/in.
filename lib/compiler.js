  ...
  return promise(function(resolve, reject, notify) {
    ...
    try {
      ...
      if (expansion.eval.match(/expand\./)
        ...
        // js = spandex.poop();  // out.as.done fixed bug 1
        js = spandex.pop();
        seq = 0;
        ...
        return sequence(spandex.map(function(x) {
          return function() {
            return promise(function(resolve, reject, notify) {
              ...
              eval('promised = ' + x.origEval.replace(x.fullName, 'expanderFn'));
              
              if (typeof promised === 'undefined') return resolve($e[seq++] = null);
              if (typeof promised.then !== 'function') return resolve($e[seq++] = promised);
          ...
          function() {
            ...
            $e.reverse() // um. inbound trickyness
    ...
    if ((m = bld.match(/^expand\.(.*)\(/)) || (m = bld.match(/^(async)\(/))) {
      ...
      for (var j = 0; j < bld.length; j++) {
        ...
        if (bld[j] == '(') {
          ...
          if (depth == 0) console.log({ERM: orig})
          depth--;
          if (depth == 0) break;
