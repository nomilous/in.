...
var _promise = require('when').promise;
  ...
  return _promise(function(Resolve, Reject, Notify) {
    ...
    try {
      ...
      if (expansion.eval.match(/\$\$/)) {
        ...
        return _sequence(spandex.map(function(x) {
          return function() {
            return _promise(function(Resolve, Reject, Notify) {
              ...
              if (asJs) eval(run);
              else eval(_coffee.compile(run, {bare: true}));
      ...
      else eval(_coffee.compile('capture('+expansion.eval+')', {bare: true}));
