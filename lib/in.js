...
function In() {
  ...
  // Masquerade run() as original function without infused or special args
  // (for external injectors)

  run.toString = function() {

    if (newFnString) return newFnString;

    newFnString = inFnString;
    
    var keepArgs = Object.keys(inArgs)

    .map(function(argName) {
      return inArgs[argName];
    })

    .filter(function(arg) {
      if (arg.infuse) return false;
      if (opts.$$specialArgs.indexOf(arg.name) >= 0) return false;
      return true;
    })
    ...
    var keepArgs = util.format('(%s)', keepArgs.join(', '));
    newFnString = newFnString.replace(/\((.|\n)*?\)/, keepArgs);
    return newFnString;
