...
Format.perform = function(opts, accum, arg) {
  return promise(function(resolve, reject, notify) {
    ...
    var match, action, split, actionPlusAdapter;
    ...
    if (match = arg.infuse.match(/^in\.\s+(.*?)\s+(.*)/)) {

      action.action = 'as';
      action.adapters = [];
      action.actor = match[1];
      // action.expansion = match[2]; // looses multiline
      action.expansion = arg.infuse.replace(new RegExp('^in.\s+'+action.actor), '').trim();

    } else if (match = arg.infuse.match(/^in\.(.*?)\s(.*?)\s/)) {

      actionPlusAdapter = match[1];
      action.action = match[1];
      extractAdapter();
      action.actor = match[2];
      action.expansion = arg.infuse.replace(new RegExp('^in.'+actionPlusAdapter+' '+action.actor), '').trim();

    } else if (match = arg.infuse.match(/^in\.(.*?)\s(.*?)$/)) {
      action.action = match[1];
      extractAdapter();
      action.actor = match[2];
      action.expansion = undefined;

    } else if (match = arg.infuse.match(/^in.(.*)$/)) {
      action.action = match[1];
      extractAdapter();
      action.actor = 'none';
      action.expansion = undefined;
    ...
    action.action = ['in', action.action];
