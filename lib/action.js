...
Action.perform = function(defer, opts, accumulate, arg) {
  ...
  var actor;
  var actionArgs = [];
  var actorPath;  // this will become a problem if support in.{{dynamic-action}}.{{dynamic-filter}}
  ...
  sequence(arg.actions.map(function(action) {
    ...
    function(results) {
      // if (typeof actor.onExpanded == 'function') {
                                              
      //                                     // questionable? may vanish
      //                                    //  
      //   return defer.resolve(actor.onExpanded(arg, actionArgs, expansions, results));
      // }
      filter.perform(opts, accumulate, arg, results).then(
...
Action.validate = function(action, actor, actorName) {
  ...
  if (action.filters.indexOf('pipe') > 0)
    throw new InfusionError('pipe must be first filter');
  ...
  if (typeof actor.$$can === 'undefined')
    throw new InfusionError(actorName + ' is not filter aware. missing actor.$$can()');

  if (!actor.$$can(action))
    throw new InfusionError(actorName + ' does not support filter combination');
