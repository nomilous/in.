      ...
      return promise(function(resolve, reject, notify) {
        ...
        try {
          ...
          if (typeof actor !== 'function') {
            return reject(new InfusionError('Actor at \`'+ actionArg.actor +'\' is not a function.'))
          }
                                                       // remaing path
          Action.validate(action, base, actorName);   //
          promise = actor(opts, inArgs, actionArg, actorPath);
        ...
        function() {
          try {
            if (arg.asArray) { // per for loop presence
                              // expanders/actors will need to set it
                             // 
                            // and async/callback??
